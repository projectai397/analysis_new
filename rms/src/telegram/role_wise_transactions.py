# src/telegram/role_wise_transactions.py
...
from src.config import transactions, users  # ✅ transactions collection + users collection
...
ENTITY_PAGE_SIZE = 5  # ✅ same like users list

RWT_TX_CACHE: Dict[int, Dict[str, Any]] = {}
RWT_TX_PAGE_SIZE = 5  # ✅ requested: 5 transactions per page
...
def resolve_user_display(user_id: ObjectId) -> str:
    """
    Convert transaction.userId(ObjectId) to readable user name/userName/username/phone.
    """
    try:
...
def _role_title(rn: str) -> str:
    if rn == "superadmin":
        return "Role-wise Transactions (Superadmin)"
    if rn == "admin":
        return "Role-wise Transactions (Admin)"
    if rn == "master":
        return "Role-wise Transactions (Master)"
    return "Role-wise Transactions"
...
def _load_clients_under_entity(entity_category: str, entity_oid: ObjectId) -> List[Dict[str, Any]]:
    """
    Given an admin/master ObjectId, return clients under that entity.
    """
...
def _query_transactions_for_user_ids(user_ids: List[ObjectId]) -> List[Dict[str, Any]]:
    """
    Pull today's transactions for the given client userIds.
    """
...
def _build_tx_page(tg_id: int) -> Tuple[str, List[List[InlineKeyboardButton]]]:
    cache = RWT_TX_CACHE.get(tg_id)
    if not cache:
        return "No transactions cached.", []

    items: List[Dict[str, Any]] = cache["items"]
    page: int = cache["page"]
    header_title: str = cache.get("header_title", "💰 <b>Transactions (today)</b>")

    if not items:
        return f"{header_title}\n\n💰 No transactions for today.", []

    total = len(items)
    max_page = (total - 1) // RWT_TX_PAGE_SIZE
    page = max(0, min(page, max_page))
    cache["page"] = page

    start_i = page * RWT_TX_PAGE_SIZE
    end_i = start_i + RWT_TX_PAGE_SIZE
    ...
    for tx in chunk:
        tid = str(tx.get("_id"))
        symbol = tx.get("symbolName") or tx.get("symbolTitle") or str(tx.get("symbolId") or "—")
        amount = tx.get("amount") or 0
        ttype = tx.get("transactionType") or tx.get("type") or ""
        label = f"{symbol} | {ttype} {amount}"
        keyboard.append([InlineKeyboardButton(label, callback_data=f"rwt_tx_detail:{tid}")])

    nav_row: List[InlineKeyboardButton] = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("⬅ Prev", callback_data="rwt_tx_page:prev"))
    if end_i < total:
        nav_row.append(InlineKeyboardButton("Next ➡", callback_data="rwt_tx_page:next"))
    if nav_row:
...
def _build_entity_list_page(tg_id: int) -> Tuple[str, List[List[InlineKeyboardButton]]]:
    ...
    for u in chunk:
        ...
        keyboard.append([InlineKeyboardButton(label, callback_data=f"rwt_entity:{category}:{uid}")])

    nav_row: List[InlineKeyboardButton] = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("⬅ Prev", callback_data="rwt_entity_page:prev"))
    if end_i < total:
        nav_row.append(InlineKeyboardButton("Next ➡", callback_data="rwt_entity_page:next"))
    if nav_row:
        keyboard.append(nav_row)
...
# ---------- /role_wise_transaction ----------
async def role_wise_transaction_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rn == "superadmin":
        buttons = [
            [InlineKeyboardButton("👑 Admins", callback_data="rwt_menu:admin")],
            [InlineKeyboardButton("🧩 Masters", callback_data="rwt_menu:master")],
            [InlineKeyboardButton("👤 Clients", callback_data="rwt_menu:client")],
        ]
    elif rn == "admin":
        buttons = [
            [InlineKeyboardButton("🧩 Masters", callback_data="rwt_menu:master")],
    ...
    buttons.append([InlineKeyboardButton("🔍 Search Client", callback_data="rwt_search:start")])
...
# ---------- menu → list entities ----------
async def rwt_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def rwt_entity_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
# ---------- entity selected → load transactions ----------
async def rwt_entity_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    # If entity is client: transactions only for that client
    if entity_category == "client":
        client_ids = [entity_oid]
        header_title = "💰 <b>Transactions (today)</b>\n" + format_entity_header("Client", entity_id)
    else:
        try:
    ...
    try:
        items = _query_transactions_for_user_ids(client_ids)
    except Exception as e:
        logger.error(f"transactions query error: {e}")
        msg = await query.message.reply_text("⚠ Error while loading transactions.")
        remember_bot_message_from_message(update, msg)
        return
    ...
    text, keyboard = _build_tx_page(tg_id)
...
async def rwt_tx_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def rwt_tx_detail_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        logger.error(f"rwt_tx_detail find_one error: {e}")
    ...
    # ✅ Show User name instead of ObjectId
    field_order = [
        ("Trade ID", "tradeId"),
    ...
    header = "💰 <b>Transaction Summary</b>\n\n<pre>" + table_text + "</pre>"
...
async def rwt_search_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    context.user_data["rwt_search_mode"] = True
    ...
    remember_bot_message_from_message(update, msg)
    context.user_data["rwt_search_prompt_msg_id"] = msg.message_id
...
def _build_search_page(tg_id: int) -> Tuple[str, List[List[InlineKeyboardButton]]]:
    ...
    for u in chunk:
        uid = str(u.get("id") or u.get("_id"))
        label = f"{display_name(u)} ({u.get('phone') or ''})"
        keyboard.append([InlineKeyboardButton(label, callback_data=f"rwt_entity:client:{uid}")])

    nav_row: List[InlineKeyboardButton] = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("⬅ Prev", callback_data="rwt_search_page:prev"))
    if end_i < total:
        nav_row.append(InlineKeyboardButton("Next ➡", callback_data="rwt_search_page:next"))
    if nav_row:
        keyboard.append(nav_row)
...
async def rwt_search_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get("rwt_search_mode"):
        return

    context.user_data["rwt_search_mode"] = False
    ...
    prompt_id = context.user_data.pop("rwt_search_prompt_msg_id", None)
    if prompt_id is not None:
        await safe_delete_message(context.bot, chat_id, prompt_id)

    if not term:
        msg = await update.effective_chat.send_message(
            "⚠ Empty search. Use /role_wise_transaction and tap <b>Search Client</b> again.",
            parse_mode="HTML",
        )
        remember_bot_message_from_message(update, msg)
        return
...
async def rwt_search_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
# ---------- register ----------
def register_role_wise_transaction_handlers(app):
    app.add_handler(CommandHandler(["role_wise_transaction", "role_wise_transactions"], role_wise_transaction_cmd))

    app.add_handler(CallbackQueryHandler(rwt_menu_callback, pattern=r"^rwt_menu:"))
    app.add_handler(CallbackQueryHandler(rwt_entity_page_callback, pattern=r"^rwt_entity_page:"))
    app.add_handler(CallbackQueryHandler(rwt_entity_select_callback, pattern=r"^rwt_entity:"))

    app.add_handler(CallbackQueryHandler(rwt_tx_page_callback, pattern=r"^rwt_tx_page:"))
    app.add_handler(CallbackQueryHandler(rwt_tx_detail_callback, pattern=r"^rwt_tx_detail:"))

    app.add_handler(CallbackQueryHandler(rwt_search_start_callback, pattern=r"^rwt_search:start$"))
    app.add_handler(CallbackQueryHandler(rwt_search_page_callback, pattern=r"^rwt_search_page:"))

    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, rwt_search_text_handler))
