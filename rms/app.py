...
if __name__ == "__main__":
    ...
    schedule.every().day.at("18:52").do(
    ...
    schedule.every().day.at("18:56").do(
    ...
    schedule.every().day.at("18:58").do(
...
#     schedule.every().day.at("18:39").do(
#         lambda: _run_async(_daily_backup_job, "backup-03")
#     )
...
#                     f"✔ Daily upload done → s3://{res_up['bucket']}/{res_up['key']}"
#                 )
#                 # Trigger download only if the upload is successful
#                 res_download = upload_backup_to_s3(
#                     date_str=None,
#                     out_root="backups",
#                     bucket=None,
#                     s3_prefix="mongo_backup",
#                     download_after_upload=True,  # Enable download after upload
#                     local_download_dir="downloads"  # Specify the directory for downloading
#                 )
#                 if res_download.get("ok"):
#                     logger.info(f"✔ Daily download done → {res_download['downloaded_to']}")
#                 else:
#                     logger.error(f"✖ Daily download failed → {res_download.get('error')}")
#             else:
#                 logger.error(f"✖ Daily upload failed → {res_up.get('error')}")
#         except Exception as e:
#             logger.exception(f"✖ Daily upload crashed: {e}")
...
#     def _daily_download_job():
#         try:
#             # Add the download step after the upload job
#             res_download = upload_backup_to_s3(
#                 date_str=None,
#                 out_root="backups",
#                 bucket=None,
...
#             if res_download.get("ok"):
#                 logger.info(
#                     f"✔ Daily download done → {res_download['downloaded_to']}"
#                 )
#             else:
#                 logger.error(f"✖ Daily download failed → {res_download.get('error')}")
#         except Exception as e:
#             logger.exception(f"✖ Daily download crashed: {e}")

#     # Schedule the download job to run shortly after the upload job
#     schedule.every().day.at("18:47").do(
#         lambda: _run_async(_daily_download_job, "download-06")
#     )
...
#     schedule.every().day.at("18:50").do(
#         lambda: _run_async(_daily_cleanup_job, "cleanup-05")
#     )
